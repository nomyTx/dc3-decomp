...
#include <cstddef>
    ...
    && (defined(_STLP_DEBUG) || defined(_STLP_ASSERTIONS) || defined(_STLP_DEBUG_ALLOC))
...
_STLP_BEGIN_NAMESPACE
    ...
    public:
        ...
        // __n is permitted to be 0.  The C++ standard says nothing about what the return
        // value is when __n == 0.
...
#endif
                MemFree(__p, __FILE__, 0xAC, "__new_alloc");
        ...
        template <class _T2>
        bool operator==(const allocator<_T2> &) const _STLP_NOTHROW {
            return true;
        }
        template <class _T2>
        bool operator!=(const allocator<_T2> &) const _STLP_NOTHROW {
...
#endif
#if defined(__MRC__) || (defined(__SC__) && !defined(__DMC__)) //*ty 03/24/2001 - MPW
                                                               // compilers get confused
                                                               // on these operator
                                                               // definitions
        template <class _T2>
        bool operator==(const allocator<_T2> &) const _STLP_NOTHROW {
    ...
    inline bool _STLP_CALL operator==(const allocator<_T1> &, const allocator<_T2> &)
        _STLP_NOTHROW {
        return true;
    }
    template <class _T1, class _T2>
...
#define _STLP_FORCE_ALLOCATORS(a, y)
    ...
    // The version for the default allocator, for rare occasion when we have partial spec
    // w/o member template classes
    ...
    inline _STLP_TYPENAME_ON_RETURN_TYPE _Alloc_traits<_Tp, _Alloc>::allocator_type
        _STLP_CALL __stl_alloc_create(const _Alloc &__a, const _Tp *) {
...
#include <stl/_alloc_old.h>
    ...
    public:
        ...
        _STLP_alloc_proxy(__move_source<_Self> src)
            : _MaybeReboundAlloc(_AsMoveSource<_Base>(src.get())),
              _M_data(_AsMoveSource<_Value>(src.get()._M_data)) {}
